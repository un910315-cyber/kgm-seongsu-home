...
window._wcSwitchView=function(view){var home=document.getElementById('estimateWorkCodeHome'),vehicle=document.getElementById('estimateVehicleView');home.style.display=view==='codes'?'block':'none';vehicle.style.display=view==='vehicle'?'block':'none';document.querySelectorAll('.estimate-view-tab').forEach(function(b){b.classList.toggle('active',b.dataset.view===view);});if(view==='codes')renderHome();};
function build(){
 ...
 var tabs=document.createElement('div');tabs.className='estimate-view-tabs';tabs.innerHTML='<button class="estimate-view-tab active" data-view="codes" onclick="window._wcSwitchView(\'codes\')">작업코드 참고</button><button class="estimate-view-tab" data-view="vehicle" onclick="window._wcSwitchView(\'vehicle\')">차량 모형 견적</button>';
 ...
 page.appendChild(tabs);page.appendChild(home);page.appendChild(vehicle);
