...
// Firebase 초기화 + 인증 + 거래·연차·게시판·블랙리스트 등 메인 로직
  ...
  // (setPersistence 블록 제거 — Firebase 기본 persistence 사용으로 되돌림)
  ...
  // 매출 추이 카드 하단 — 좌: 수리 대기 / 우: 수리 완료 (각각 오래된 입고순)
  function renderTrendWaitList(list) {
    ...
    const col = (mod, dot, title, rows, emptyMsg) => {
      const head = '<div class="trend-wait-head"><span class="tw-dot" style="background:' + dot + ';"></span>' + title + '</div>';
      const body = rows.length ? rowsHtml(rows) : '<div class="trend-wait-empty">' + emptyMsg + '</div>';
      return '<div class="trend-wait-col ' + mod + '">' + head + body + '</div>';
    };

    box.innerHTML =
      col('tw-waiting', '#a78bfa', '수리 대기 ' + waiting.length + '대', waiting, '대기 중인 차량 없음')
      + col('tw-finished', '#34d399', '수리 완료 ' + done.length + '대', done, '완료된 차량 없음');
  }
  ...
  function renderReservationBoard(){
    ...
    var list=all.filter(function(i){return (tf==='all'||i.type===tf)&&(sf==='all'||(sf==='active'?i.status!=='done':i.status===sf))&&(!q||[i.title,i.carNum,i.customer,i.phone,i.memo].join(' ').toLowerCase().includes(q));});
    list.sort(function(a,b){if((a.status==='done')!==(b.status==='done'))return a.status==='done'?1:-1;return ((a.dueDate||'9999')+' '+(a.dueTime||'')).localeCompare((b.dueDate||'9999')+' '+(b.dueTime||''));});
    var box=document.getElementById('rv-list'),empty=document.getElementById('rv-empty');if(!box||!empty)return;empty.style.display=list.length?'none':'block';
